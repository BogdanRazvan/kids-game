...
// --- Content-address every clip ---------------------------------------------
// Up to here a clip is named after its TEXT, so "roșu" kept the same file name
// forever — even after you re-recorded it. Hosting serves these mp3s as
// `immutable, max-age=1y`, so browsers/CDNs/service workers happily served the
// OLD voice for a year. Renaming each file after a hash of its BYTES means new
// audio always gets a new URL, so a re-recording can never be served stale (and
// makes the `immutable` header actually correct). Identical clips dedupe.
for (const [text, file] of Object.entries(manifest)) {
  const src = join(AUDIO_DIR, file)
  const name = createHash('sha1').update(readFileSync(src)).digest('hex').slice(0, 16) + '.mp3'
  if (name === file) continue
  const dest = join(AUDIO_DIR, name)
  if (existsSync(dest)) rmSync(src) // same audio as another phrase — share it
